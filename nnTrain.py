...
# flag = 1 时 读取mnist数据集并训练保存模型
# flag = 0 时 读取mnist测试数据集打印正确率
flag = 0
...
# # 定义函数以方便构造网络
# 初始化权重,传入大小参数，truncated_normal函数使得w呈正态分布，
# stddev设置标准差为0.1。也就是说输入形状大小，输出正态分布的随机参数作为权重变量矩阵
...
if flag :
    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            ...
            print("test accuracy %g" % accuracy.eval(feed_dict={x: testSet[0], y_: testSet[1], keep_pro: 1.0}))
